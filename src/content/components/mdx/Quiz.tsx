 ...
 * @param children
 */
export const Quiz: React.FC<{ children?: React.ReactNode }> = ({ children }) => {
...
export const QuizOption: React.FC<{ children?: React.ReactNode } & { isCorrect: boolean; i: any }> = option => {
  const quizContext = useQuizContext();
...
export const _QuizModalBody = () => {
  ...
  async function submitAnswer() {
    if (selected !== undefined) {
      setSubmitted(true);
      try {
        await UserService.completeQuiz(courseId, `${chapter}.${pageId}`, selected === correct);
      } catch (e) {
        ErrorHandlingService.notifyUserOfError(e, "Error submitting quiz");
      }
    }
  }
