 ...
 * @param children
 */
export const Challenge: React.FC<{ children?: React.ReactNode }> = ({ children }) => {
...
const _ChallengePreview = () => {
  ...
  const color = DatabaseStore.database$.value.allCourses[courseId].color + ".600";
...
export const ChallengeDescription: React.FC<{ children?: React.ReactNode }> = ({ children }) => {
  const { description } = useChallengeContext();
  description[1](children);
  return <></>;
};

export const ChallengeAnswer: React.FC<{ children?: React.ReactNode }> = ({ children }) => {
...
export const _ChallengeModalBody = () => {
  ...
  async function submitAnswer() {
    setSubmitted(true);
    try {
      await UserService.completeChallenge(courseId, `${chapter}.${pageId}`);
    } catch (e) {
      ErrorHandlingService.notifyUserOfError(e, "Error submitting challenge");
    }
  }
